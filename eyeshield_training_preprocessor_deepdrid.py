...
def _ensure_dependencies():
    """Ensure required local modules exist before running."""
    required_files = {
        'image_processor.py': 'https://raw.githubusercontent.com/dondondon22/EyeShield/refs/heads/main/image_processor.py'
    }
    for file, url in required_files.items():
        if not os.path.exists(file):
            print(f"Downloading required dependency: {file}...")
            try:
...
def resolve_dataset_root(df):
    ...
    # 2) kagglehub + common nested layouts for DeepDRiD
    try:
        import kagglehub
        kagglehub_root = kagglehub.dataset_download("nancyhisham/deepdrid")
        candidate_roots.extend([
            kagglehub_root,
            os.path.join(kagglehub_root, 'regular_fundus_images'),
        ])
    except Exception as e:
        pass
    ...
    # Score candidate roots across many CSV image paths and pick the highest-coverage root
    rel_paths = df['image_path'].astype(str).str.lstrip('/\\')
    probe_n = min(len(rel_paths), 64)
    ...
    for root in unique_roots:
        score = sum(os.path.exists(os.path.join(root, p)) for p in probe_paths)
        if score > best_score:
            best_score = score
            best_root = root
            
        if score == probe_n: # perfect score, can stop early
            break

    if best_score > 0 and best_root is not None:
        print(f"? Dataset root resolved: {best_root} (score: {best_score}/{probe_n})")
        return best_root

    # 4) Fallback search (bounded depth)
    search_bases = ['/kaggle/input', '/root/.cache/kagglehub', '/content/dataset']
    for base in search_bases:
        ...
        for current_root, dirs, _ in os.walk(base):
            ...
            score = sum(os.path.exists(os.path.join(current_root, p)) for p in probe_paths)
            if score > 0:
                print(f"? Dataset root discovered via search: {current_root} (score: {score}/{probe_n})")
                return current_root

    raise FileNotFoundError(
        "Could not resolve dataset root for CSV paths.\n"
        f"Sample relative path: {probe_paths[0] if probe_paths else 'Unknown'}\n"
